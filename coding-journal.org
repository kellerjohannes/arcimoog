...
Musical notation depends on many conventions. I try to summarize them
in order to design a suitable data structure, maintaining the two
dimensional space as a starting point. Both the time and the pitch
dimension are quantized, which means that there is a predefined set of
discrete values for both coordinates.
...
note glyph. There would be many more, like the direction of the stem,
the length and thickness of the stem, etc., but those don't change the
musical meaning of the glyph. The struct doesn't include any time
context, like the time signature, or place within a bar or beat.
...
~note-value~ and ~pitch~ for as coordinates.
...
structure contains the relevant properties of notes. A conversion to
Lilypond code would be trivial. For each property there needs to be a
dictionary in order to translate the data into valid Lilypond syntax.
...
(defun data-to-lilypond (data)
  (let ((result (format nil "\\version \"2.22.2\"~%~%{ ")))
    (dolist (glyph data)
...
by a visual representation. Pitch and time should be calculated so
that an audio engine could be fed with them, but instead of an actual
audio processor I'll feed a graphical backend that will produce a two
dimensional diagram displaying time on the x axis and frequency on the
y axis.

I'll focus first on the tranformation of the musical data into
absolute values, and on the graphical output later. This
transformation requires some music theoretical ground rules. The
mapping between pitch letters with their alterations to frequencies is
defined by a tuning system, or scale.
...
I'd like to keep this flexible, so instead of a dictionary I'll
express the scale as a function. That allows me to experiment with
different functions and generate different outputs. To write this
function, I need to implement a model of a tuning system. There are
various models I encountered so far:
...
I tried a recursive approach, since I know that I will need to add a
...
Also, the previous implementation of ~linear-system~ didn't process
negative indices. A negative index is convenient if I would like to
describe a chain of fifths starting from a central note with
index 0. Negative indices cause the chain to expand "to the left",
adding descending fifths to the system.
