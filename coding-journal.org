...
accidental needs to be a keyword or nil."
  (and (member (letter note) (list :a :b :c :d :e :f :g))
       (or (keywordp (accidental note))
           (null (accidental note))
           (and (listp (accidental note))
                (every #'keywordp (accidental note))))
       (call-next-method)))

(defmethod validp ((note note-name-smn))
  "Checks validity for accidentals."
  )
#+end_src

#+RESULTS:
: #<STANDARD-METHOD COMMON-LISP-USER::VALIDP (NOTE-NAME-SMN) {1009419F93}>
