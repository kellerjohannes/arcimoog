  ...
  (let ((time-cursor 0)
    ...
    (flet ((generate-interval-result ()
      ...
      (dolist (melody-item (parse-melody-data melody-data) (values (generate-interval-result)
        ...
        (case (first melody-item)
          (:i
           (push (second melody-item) interval-path))
          (:s
           (incf time-cursor (second melody-item))
           (setf gate-cursor t))
          (:t
           (incf time-cursor (second melody-item))
           (setf gate-cursor nil)))
        (when (>= time-cursor timecode)
...
(defun probe-score (tree-name score-data timecode)
  (let ((interval-to-origin (mapcar (lambda (voice)
                                      (list (first voice)
                                            (multiple-value-bind (interval gatep)
                                                (ff-voice tree-name (rest voice) timecode)
                                              (if gatep (list :absolute interval) nil))))
                                    score-data)))
    (let ((result))
      (dolist (voice interval-to-origin (reverse result))
        (when (second voice)
          (push `(,(first voice)
                  ,(second voice)
                  ,@(remove-if #'null
                               (mapcar (lambda (reference-voice)
                                         (when (and (not (eq (first reference-voice)
                                                             (first voice)))
                                                    (second reference-voice))
                                           (list :relative-to
                                                 (first reference-voice)
                                                 (condense
                                                  (list (second (second voice))
                                                        (mirror (second (second reference-voice))))
                                                  (get-interval-tree tree-name)
                                                  (get-identity-interval tree-name)))))
                                       interval-to-origin)))
                result))))))
